...
use framework\database\IDriver;
...
class Pdo implements IDriver {
    ...
    public function connection($serverType) {
        $server = Config::getDatabase($this->_configName)->getServer($serverType);
    ...
    public function execute($closeStatement = false) {
        $db = Database::getDatabase($this->_configName);
        if ($db::getDebug())
            Benchmark::getInstance($this->_configName)->startTime(2)->startRam(2);
        ...
        if ($db::getDebug()) {
            ...
            $parameters = '';
            foreach ($this->_params as $param)
                $parameters .= (string) $param['value'] . ' (Type ' . $this->_paramTypeName[$param['type']] . ') ';
            ...
            $this->_logs[] = 'Query : ' . $this->_query . ' with parameters values : "' . trim($parameters, ' ') . '" Time : ' . $time . ' milli-seconds Ram : ' . $ram . ' kilo-octets Error : ' . $errorMessage;
