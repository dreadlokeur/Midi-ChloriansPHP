...
abstract class Model {
    ...
    public static function factoryManager($name, $dbName, $dbTable) {
        ...
        $manager->setAdaptater($manager->getDb(true));
        return $manager;
    }
    ...
    public function getDb($returnAdaptater = false) {
        return Database::getDatabase($this->_modelDBName, $returnAdaptater);
    }
    ...
    public function execute($query, $parameters = array(), $returnLastInsertId = false, $closeStatement = false, $checkBindNumber = true) {
        $this->_adaptater->set($query);
        foreach ($parameters as $paramValue => $paramType)
            $this->_adaptater->bind($paramValue, $paramType);

        $this->_adaptater->execute($closeStatement, $checkBindNumber);


        if ($returnLastInsertId)
            return $this->_adaptater->lastInsertId();
    }
