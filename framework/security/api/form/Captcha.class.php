...
namespace framework\security\api\form;

use framework\security\api\IForm;
...
class Captcha implements IForm {
    ...
    public function __construct($options = array()) {
        if (isset($options['dataFile'])) {
            if (!file_exists($options['dataFile']) || !Validate::isFileMimeType('xml', $options['dataFile']))
                throw new \Exception('Api captcha invalid data file');

            $xml = @simplexml_load_file($options['dataFile']);
            if ($xml === null || $xml === false)
                throw new \Exception('Api captcha invalid data xml file');
        ...
        $this->_refreshUrl = isset($options['refreshUrl']) ? $options['imageUrl'] : 'captcha';
    ...
    public function get($type = 'image', $url = false) {
        if ($type == 'image') {
            if (!$this->_image)
                throw new \Exception('Api captcha image parameter not allowed');

            if ($url)
                return Config::getUrl($this->_imageUrl, true, array($this->getFormName(), 'image'));

            $this->_display($type);
        } elseif ($type == 'audio') {
            if (!$this->_audio)
                throw new \Exception('Api captcha audio parameter not allowed');
            if ($url)
                return Config::getUrl($this->_imageUrl, true, array($this->getFormName(), 'audio'));
    ...
    public function addAttempt($attemptInfo = '') {
        if (Api::getDebug())
            Logger::getInstance()->debug('Captcha : "' . $this->getFormName() . '" attempt, information : "' . $attemptInfo . '"', 'securityApi');
    }
    ...
    public function getRefreshUrl() {
        return Config::getUrl($this->_refreshUrl, true, array($this->getFormName(), 'refresh'));
    }
    ...
    public function setImage($activate, $options = array()) {
        ...
        if ($activate) {
            if (!extension_loaded('gd'))
                throw new \Exception('Api captcha need GD extension');

            if (!isset($options['imageFormat']) || !in_array(strtolower($options['imageFormat']), $this->_imageFormatList))
                throw new \Exception('Api captcha need a valid image format');
            $this->_imageFormat = strtolower($options['imageFormat']);

            $this->_imageUrl = isset($options['imageUrl']) ? $options['imageUrl'] : 'captcha';

            // Image Size
            if (!isset($options['imageWidth']))
                throw new \Exception('Api captcha need a imageWidth value');
            $this->setImageWidth($options['imageWidth']);
            if (!isset($options['imageHeight']))
                throw new \Exception('Api captcha need a imageHeight value');
            $this->setImageHeight($options['imageHeight']);

            // Font
            if (!isset($options['imageFontFile']))
                throw new \Exception('Api captcha need a font file');
    ...
    public function setAudio($activate, $options = array()) {
        ...
        if ($activate) {
            $this->_audioUrl = isset($options['audioUrl']) ? $options['audioUrl'] : 'captcha';
            if (!isset($options['audioLangDirectory']))
                throw new \Exception('Api captcha need a audio lang directory parameter');
            ...
            if (isset($options['audioNoise'])) {
                ...
                if (!isset($options['audioNoiseFile']))
                    throw new \Exception('Api captcha need a audio noise need audio noise file parameter');
    ...
    protected function _createAudio() {
        ...
        try {
            ...
            if (Api::getDebug())
                Logger::getInstance()->debug('Api captcha generate audio file for form : "' . $this->getFormName() . '" error : "' . $e . '"', 'securityApi');
