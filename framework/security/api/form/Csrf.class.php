...
class Csrf implements IForm {
    ...
    public function set() {
        if (is_null($this->_token)) {
            if (Api::getDebug())
                Logger::getInstance()->debug('Crsf : "' . $this->getFormName() . '" tryint set uncreated token', 'securityApi');
            return;
        }
    ...
    public function addAttempt($attemptInfo = '') {
        if (Api::getDebug())
            Logger::getInstance()->debug('Crsf : "' . $this->getFormName() . '" attempt, information : "' . $attemptInfo . '"', 'securityApi');
    }
