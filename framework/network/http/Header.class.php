...
class Header {
    ...
    public static function sentHeader($directiveName, $directiveValue, $replace = true, $forceResponseCode = false, $checkIfHeaderSent = true, $directiveSeparator = ':') {
        ...
        if ($forceResponseCode && !ResponseCode::existsCode($forceResponseCode))
        ...
        if ($checkIfHeaderSent) {
            if (!headers_sent())
                self::_sentHeader($directiveName, $directiveValue, $replace, $forceResponseCode, $directiveSeparator);
            else
    ...
    protected static function _sentHeader($directiveName, $fieldValue, $replace, $forceResponseCode, $directiveSeparator) {
        if (is_null($directiveSeparator)) {
            header($directiveName . $fieldValue, $replace, $forceResponseCode);
        }
        else
            header($directiveName . ': ' . $fieldValue, $replace, $forceResponseCode);
    }
    ...
    public static function setResponseStatusCode($code, $sentHttpStatus = false, $checkIfHeaderSent = true, $httpProtocol = null) {
        if (!ResponseCode::existsCode($code))
        ...
        if ($sentHttpStatus) {
            $httpProtocolVersion = (!is_null($httpProtocol) && Http::existsHttpProtocolVersion($httpProtocol)) ? $httpProtocol : str_replace('HTTP/', '', Http::getServer('SERVER_PROTOCOL'));
            $statusMessage = ResponseCode::getMessageForCode($code, false);
